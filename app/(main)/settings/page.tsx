  ...
  return (
    <div ref={ref} className="relative inline-block text-left">
      <button
        ...
        className={cn(
          "inline-flex items-center justify-between rounded border border-border/45 bg-card px-2.5 py-1 text-[12px]",
          "hover:bg-muted/60 transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary/30"
        )}
      ...
      {open && (
        <div className="absolute right-0 z-20 mt-1 w-full min-w-[120px] rounded border border-border/45 bg-card py-0.5 shadow-md animate-in fade-in zoom-in-95 origin-top-right">
          {options.map((opt) => (
            <button
              key={opt}
              type="button"
              ...
              className={cn(
                "block w-full px-2.5 py-1.5 text-left text-[12px] hover:bg-muted/60 transition-colors",
                opt === value && "font-medium text-foreground/90"
              )}
            >
              {opt}
            </button>
  ...
  return (
    <div className="flex items-center justify-between gap-8 py-2.5">
      <div className="min-w-0">
        ...
        {description && <div className="text-[11px] text-muted-foreground/65 mt-0.5">{description}</div>}
      </div>
...
export default function SettingsPage() {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-background">
      <div className="max-w-[580px] mx-auto pt-10 pb-20 px-8">
          <h1 className="text-[15px] font-semibold tracking-tight text-foreground/90 mb-6 pb-4 border-b border-border/35">{sectionTitles[activeSection]}</h1>
            ...
            <section>
              <div className="flex items-center gap-3 mb-6 pb-4 border-b border-border/30">
                <Avatar className="size-10">
                  <AvatarFallback className="text-sm font-medium bg-muted/60 border border-border/40">A</AvatarFallback>
                </Avatar>
                <div>
                  <h2 className="text-[13px] font-semibold text-foreground/90">admin</h2>
                  <p className="text-[11px] text-muted-foreground/65">admin@example.com</p>
                </div>
              </div>
          ...
          {activeSection === "preferences" && (
            <section>
              ...
              <div className="flex items-center justify-between mt-6 pt-4 border-t border-border/30">
                ...
                <button onClick={handleSavePref} disabled={prefSave.status === "saving"} className="ml-auto h-7 px-3 flex items-center gap-1.5 text-[11px] font-medium rounded border border-border/40 bg-card hover:bg-muted/60 disabled:opacity-50 transition-all duration-150">
          ...
          {activeSection === "llm" && (
            <section>
              ...
              <div className="flex items-center justify-between mt-6 pt-4 border-t border-border/30">
                ...
                <button onClick={handleSaveLlm} disabled={llmSave.status === "saving"} className="ml-auto h-7 px-3 flex items-center gap-1.5 text-[11px] font-medium rounded border border-border/40 bg-card hover:bg-muted/60 disabled:opacity-50 transition-all duration-150">
          ...
          {activeSection === "storage" && (
            <section>
              ...
              <div className="flex items-center justify-between mt-6 pt-4 border-t border-border/30">
                ...
                <button onClick={handleSaveStorage} disabled={storageSave.status === "saving"} className="ml-auto h-7 px-3 flex items-center gap-1.5 text-[11px] font-medium rounded border border-border/40 bg-card hover:bg-muted/60 disabled:opacity-50 transition-all duration-150">
          ...
          {activeSection === "cookies" && (
            <section>
              ...
              <div className="flex items-center justify-between mt-6 pt-4 border-t border-border/30">
                ...
                <button onClick={handleSaveCookies} disabled={cookieSave.status === "saving"} className="ml-auto h-7 px-3 flex items-center gap-1.5 text-[11px] font-medium rounded border border-border/40 bg-card hover:bg-muted/60 disabled:opacity-50 transition-all duration-150">
          ...
          {activeSection === "integrations" && (
            <section>
              ...
              <div className="rounded-md border border-border/40 divide-y divide-border/25 overflow-hidden">
                {[
                  { name: "飞书", desc: "连接飞书进行消息通知和文档协作" },
                  { name: "GitHub", desc: "同步代码仓库和 Issues" },
                  { name: "Slack", desc: "接收分析完成通知" },
                ].map(({ name, desc }) => (
                  <div key={name} className="flex items-center justify-between px-4 py-3">
                    <div className="min-w-0">
                      <div className="text-[13px] font-medium text-foreground/85">{name}</div>
                      <div className="text-[11px] text-muted-foreground/60 mt-0.5">{desc}</div>
                    </div>
                    <button className="h-6 px-2.5 text-[11px] rounded border border-border/40 text-muted-foreground/70 hover:bg-muted/60 hover:text-foreground/80 transition-all duration-150">连接</button>
                  </div>
                ))}
              </div>
            </section>
          ...
          {activeSection === "updates" && (
            <section>
              <div className="flex items-center justify-between mb-6 pb-4 border-b border-border/30">
                <div>
                  <div className="text-[13px] font-medium text-foreground/85">当前版本 v1.2.0</div>
                  <div className="text-[11px] text-muted-foreground/60 mt-0.5">检查是否有新版本可用</div>
                </div>
                <button
                  onClick={() => {
                  ...
                  className="h-7 px-3 flex items-center gap-1.5 text-[11px] font-medium rounded border border-border/40 bg-card hover:bg-muted/60 disabled:opacity-50 transition-all duration-150"
                >
