// runtime/agent/tools/search-videos.ts —— 视频搜索工具（B站 API + yt-dlp）
...
const searchVideosSchema = z.object({
  ...
  source: z.enum(["bilibili", "youtube", "all"]).default("all"),
  count: z.number().int().min(1).max(10).default(5),
})

export function createSearchVideosTool(): AgentTool<typeof searchVideosSchema> {
  return {
    name: "search_videos",
    description:
      "在B站和YouTube上搜索学习视频。当用户想找某个主题的视频时使用。参数: keyword(必填,搜索关键词), source(选填:bilibili/youtube/all,默认all), count(选填,返回数量,默认5)",
    inputSchema: searchVideosSchema,
    dangerous: false,
    async execute(args) {
      const tasks: Promise<VideoSearchResult[]>[] = []
      if (args.source !== "youtube") tasks.push(searchBilibili(args.keyword, args.count))
      if (args.source !== "bilibili") tasks.push(searchYouTube(args.keyword, args.count))
      ...
      const errors: string[] = []
      if (args.source !== "youtube" && settled[0]?.status === "rejected") errors.push("B站搜索失败")
      if (args.source !== "bilibili") {
        const ytIdx = args.source === "youtube" ? 0 : 1
